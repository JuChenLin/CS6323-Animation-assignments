...
glm::mat4 Curve::rotate_cube(glm::mat4 cube_mat,int index){
    
    mat4 QuaterionMatrix = quaterion_to_matrix( control_points_quaternion[index], control_points_pos[index]);
    
    glm::mat4 rotated_cube_mat = QuaterionMatrix * cube_mat;
    
    return rotated_cube_mat;
    
}

glm::mat4 Curve::quaterion_to_matrix(glm::quat quaterion, glm::vec3 position){
    ...
    float quaternionArray[16] = { (1 - 2*y*y - 2*z*z), (2*x*y + 2*s*z), (2*x*z - 2*s*y), 0,
                                  (2*x*y - 2*s*z), (1 - 2*x*x - 2*z*z), (2*y*z + 2*s*x), 0,
                                  (2*x*z + 2*s*y), (2*y*z - 2*s*x), (1 - 2*x*x - 2*y*y), 0,
                                      position[0],     position[1],         position[2], 1  };

    mat4 quaterionMatrix = make_mat4(quaternionArray);
    
    // Solution 2 -----
//        glm::mat4 quaterionMatrix = toMat4(quaterion);
//        //mat4 QuaterionMatrix = mat4_cast(quaterion);
//
//        quaterionMatrix[3][0] = position[0];
//        quaterionMatrix[3][1] = position[1];
//        quaterionMatrix[3][2] = position[2];
